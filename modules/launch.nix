...
let
  ...
  coerceToEnv =
    ...
    in
    if isString val && parsed != null then
      if append != null then concat (env key) (concat "/" (coerceToEnv append)) else env key
    else
      val;
  ...
  systemDbusOutsidePath = concat (env "XDG_RUNTIME_DIR") (concat "/nixpak-system-bus-" instanceId);
  ...
  bwrapArgs = flatten (
    [
      ...
      (optionals config.bubblewrap.apivfs.dev [
        "--dev"
        "/dev"
      ])
      (optionals config.bubblewrap.apivfs.proc [
        "--proc"
        "/proc"
      ])
      ...
      (optionals config.system-dbus.enable [
        (bind [
          systemDbusOutsidePath
          "$XDG_RUNTIME_DIR/nixpak-system-bus"
        ])
        "--setenv"
        "DBUS_SYSTEM_BUS_ADDRESS"
        (concat "unix:path=" (coerceToEnv "$XDG_RUNTIME_DIR/nixpak-system-bus"))
      ])
  ...
  ++ config.system-dbus.args
  ++ [ "--filter" ];

  originalBwrapArgs = pkgs.writeText "bwrap-args.json" (builtins.toJSON bwrapArgs);
  bwrapArgsJson =
    if config.bubblewrap.bindEntireStore then
      originalBwrapArgs
    else
      pkgs.runCommandLocal "bwrap-args.json"
        {
  ...
  systemDbusProxyArgsJson = pkgs.writeText "xdg-system-dbus-proxy-args.json" (
    builtins.toJSON systemDbusProxyArgs
  );

  pastaArgsJson = pkgs.writeText "pasta-args.json" (builtins.toJSON config.pasta.args);

  waylandProxyArgsJson = pkgs.writeText "wayland-proxy-args.json" (
    builtins.toJSON config.waylandProxy.args
  );

  mainProgram = baseNameOf config.app.binPath;
      ...
      ''
        makeWrapper ${launcher}/bin/launcher $out${executablePath} \
          ${concatStringsSep " " (flatten [
            ...
            (optionals (
              config.dbus.enable || config.system-dbus.enable
            ) "--set XDG_DBUS_PROXY_EXE ${dbusProxyWrapper}")
            (optionals config.dbus.enable "--set XDG_DBUS_PROXY_ARGS ${dbusProxyArgsJson}")
            (optionals config.system-dbus.enable "--set XDG_SYSTEM_DBUS_PROXY_ARGS ${systemDbusProxyArgsJson}")
  ...
  extraEntrypointScripts = genAttrs config.app.extraEntrypoints (
    entrypoint:
    mkWrapperScript {
  ...
  dbusProxyWrapper = pkgs.writeShellScript "xdg-dbus-proxy-wrapper" ''
    exec ${config.bubblewrap.package}/bin/bwrap ${
      concatStringsSep " " (flatten [
        (bindRo "/etc")
        (bindRo "/nix/store")
        (bind "/var")
