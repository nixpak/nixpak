...
func startSessionHelper(ready chan bool, sessionHelperPath string) {
	...
	if conn_err != nil {
		log.Fatal("Failed to connect to systemd bus:", conn_err)
	}
	defer sdconn.Close()

	sessionconn, conn_err := godbus.SessionBus()
	if conn_err != nil {
		log.Fatal("Failed to connect to session bus:", conn_err.Error())
	}
	...
	if err != nil {
		log.Fatal("Failed to add match for NameOwnerChanged:", err)
	}
	...
	if start_err != nil {
		...
		if startnew_err != nil {
			log.Fatal("Failed to start transient session helper service:", startnew_err)
		}
	}
	...
	if name_err != nil {
		log.Fatal("Failed to check for name on session bus:", name_err)
	}
